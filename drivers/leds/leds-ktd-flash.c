...
static int led_ktd_flash_senddata(struct ktd_flash_led_data *flash_node, const unsigned char data)
{
	...
	// fill the data.
	if (data&0x80) SEND_HIGH(ctrl_gpio); else SEND_LOW(ctrl_gpio);
	if (data&0x40) SEND_HIGH(ctrl_gpio); else SEND_LOW(ctrl_gpio);
	if (data&0x20) SEND_HIGH(ctrl_gpio); else SEND_LOW(ctrl_gpio);
	if (data&0x10) SEND_HIGH(ctrl_gpio); else SEND_LOW(ctrl_gpio);
	if (data&0x08) SEND_HIGH(ctrl_gpio); else SEND_LOW(ctrl_gpio);
	if (data&0x04) SEND_HIGH(ctrl_gpio); else SEND_LOW(ctrl_gpio);
	if (data&0x02) SEND_HIGH(ctrl_gpio); else SEND_LOW(ctrl_gpio);
	if (data&0x01) SEND_HIGH(ctrl_gpio); else SEND_LOW(ctrl_gpio);
	// fill EOD
	...
	pr_err("led_ktd_flash_senddata end");
...
static void led_ktd_flash_set_flash_current(struct ktd_flash_led_data *flash_node, int curr)
{
	...
	pr_err("led_ktd_flash_set_flash_current: curr = %d, level = %d, data = %x", curr, flash_curr_level, data);
...
static void led_ktd_flash_set_torch_current(struct ktd_flash_led_data *flash_node, int curr)
{
	...
	pr_err("led_ktd_flash_set_torch_current: curr = %d, level = %d, data = %x", curr, torch_curr_level, data);
...
static void led_ktd_flash_set_flash_duration(struct ktd_flash_led_data *flash_node, int time)
{
	...
	pr_err("led_ktd_flash_set_flash_duration: time = %d, level = %d, data = %x", time, flash_dura_level, data);
						...
						enum led_brightness value)
{
	...
	pr_err("led_ktd_brightness_set: brightness value:%d\n", value);
	...
	if (value == LED_OFF)
	{
		...
		//gpio_set_value(flash_led_cdev->parent->flash_strobe->gpio, GPIO_OUT_LOW);
		//gpio_set_value(flash_led_cdev->parent->flash_tx->gpio, GPIO_OUT_LOW);
		return;
	}
	...
	if (flash_led_cdev->type == FLASH)
	{
		...
		//gpio_set_value(flash_led_cdev->parent->flash_strobe->gpio, GPIO_OUT_HIGH);
	}
	else
	{
		...
		//gpio_set_value(flash_led_cdev->parent->flash_strobe->gpio, GPIO_OUT_HIGH);
		//gpio_set_value(flash_led_cdev->parent->flash_tx->gpio, GPIO_OUT_HIGH);
	}
					...
					struct ktd_flash_led_data *flash_node)
{
	...
	if (flash_node->flash_strobe->gpio < 0) {
		...
		rc = gpio_request_one(flash_node->flash_strobe->gpio, GPIOF_OUT_INIT_LOW, flash_node->flash_strobe->label);
		if (rc) {
			pr_err("%s: Failed to request gpio %d,rc = %d\n",
