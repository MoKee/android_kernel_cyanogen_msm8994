...
/* Log */
#define TP_LOG_TAG "synaptics-fwu"
		...
		struct block_count *blkcount, struct physical_address *phyaddr)
{
	...
	for (ii = 0; ii < fwu->partitions; ii++) {
		...
		case CORE_CODE_PARTITION:
			...
			TP_LOGD("Core code block count: %d\n",
					blkcount->ui_firmware);
			break;
		case CORE_CONFIG_PARTITION:
			blkcount->ui_config = partition_length;
			phyaddr->ui_config = physical_address;
			TP_LOGD("Core config block count: %d\n",
					blkcount->ui_config);
			break;
		case DISPLAY_CONFIG_PARTITION:
			blkcount->dp_config = partition_length;
			phyaddr->dp_config = physical_address;
			TP_LOGD("Display config block count: %d\n",
					blkcount->dp_config);
			break;
		case FLASH_CONFIG_PARTITION:
			blkcount->fl_config = partition_length;
			TP_LOGD("Flash config block count: %d\n",
					blkcount->fl_config);
			break;
		case GUEST_CODE_PARTITION:
			blkcount->guest_code = partition_length;
			phyaddr->guest_code = physical_address;
			TP_LOGD("Guest code block count: %d\n",
					blkcount->guest_code);
			break;
		case GUEST_SERIALIZATION_PARTITION:
			blkcount->pm_config = partition_length;
			TP_LOGD("Guest serialization block count: %d\n",
					blkcount->pm_config);
			break;
		case GLOBAL_PARAMETERS_PARTITION:
			blkcount->bl_config = partition_length;
			TP_LOGD("Global parameters block count: %d\n",
					blkcount->bl_config);
			break;
		case DEVICE_CONFIG_PARTITION:
			blkcount->lockdown = partition_length;
			TP_LOGD("Device config block count: %d\n",
					blkcount->lockdown);
			break;
		};
	}
...
static int fwu_read_f34_v7_queries(void)
{
	...
	for (offset = 0; offset < V7_PARTITION_SUPPORT_BYTES; offset++) {
		...
		TP_LOGD("Supported partitions: 0x%02x\n",
...
static enum flash_area fwu_go_nogo(void)
{
	...
	TP_LOGI("Device config ID = 0x%02x 0x%02x 0x%02x 0x%02x\n",
			config_id[0],
			config_id[1],
			config_id[2],
			config_id[3]);
	...
	TP_LOGI("Image config ID = 0x%02x 0x%02x 0x%02x 0x%02x\n",
...
static int fwu_write_flash_configuration(void)
{
	...
	TP_LOGD("Erase flash configuration command written\n");
...
static int fwu_do_read_config(void)
{
	...
	switch (fwu->config_area) {
	case UI_CONFIG_AREA:
		TP_LOGI("config_area is UI_CONFIG_AREA\n");

		block_count = fwu->blkcount.ui_config;
		break;

	case DP_CONFIG_AREA:
		TP_LOGI("config_area is DP_CONFIG_AREA\n");
	...
	case PM_CONFIG_AREA:
		TP_LOGI("config_area is PM_CONFIG_AREA\n");
	...
	case BL_CONFIG_AREA:
		TP_LOGI("config_area is BL_CONFIG_AREA\n");
...
static int fwu_start_reflash(void)
{
	...
	if (fwu->image == NULL) {
		...
		TP_LOGD("Requesting firmware image %s\n", fwu->image_name);
		...
		char *buf, loff_t pos, size_t count)
{
	...
	TP_LOGI("memory copy = dest(%p), dest size(%d), "
		"src(%p), src_size(%d), count(%d)\n",
		buf, (int)count, fwu->read_config_buf,
		fwu->read_config_buf_size, fwu->config_size);
...
/**
 * read config data through I2C
 *
 * Usage: echo 1 > /sys/class/input/input[x]/readconfig
 */
static ssize_t fwu_sysfs_read_config_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
