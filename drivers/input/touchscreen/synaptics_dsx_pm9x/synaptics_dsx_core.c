...
/* Log */
#define TP_LOG_TAG "synaptics-core"
	...
	for (finger = 0; finger < fingers_supported; finger++) {
		...
		if (finger_status) {
			...
			if (rmi4_data->hw_if->board_data->panel_x >= 0)
				x = (x * rmi4_data->hw_if->board_data->panel_x) / 900;
			if (rmi4_data->hw_if->board_data->panel_y >= 0)
				y = (y * rmi4_data->hw_if->board_data->panel_y) / 1650;
...
/**
 * synaptics_rmi4_gpio_setup - setup GPIO
 * @gpio: the number of GPIO
 * @config: true for setup GPIO, false for free GPIO
 * @dir: 0 = input, otherwise = output
 * @state: the output state (1:HIGH / 0:LOW, only for output)
 */
static int synaptics_rmi4_gpio_setup(int gpio, bool config, int dir, int state)
{
	...
	if (config) {
		...
		if (retval) {
			TP_LOGE("Failed to set gpio %d direction", gpio);
			return retval;
		}
	} else {
		gpio_free(gpio);
...
static void synaptics_rmi4_set_params(struct synaptics_rmi4_data *rmi4_data)
{
	...
	if (rmi4_data->hw_if->board_data->panel_x >= 0)
		input_set_abs_params(rmi4_data->input_dev,
				ABS_MT_POSITION_X, 0,
				rmi4_data->hw_if->board_data->panel_x, 0, 0);
	else
		input_set_abs_params(rmi4_data->input_dev,
				ABS_MT_POSITION_X, 0,
	...
	if (rmi4_data->hw_if->board_data->panel_y >= 0)
		input_set_abs_params(rmi4_data->input_dev,
				ABS_MT_POSITION_Y, 0,
				rmi4_data->hw_if->board_data->panel_y, 0, 0);
	else
		input_set_abs_params(rmi4_data->input_dev,
				ABS_MT_POSITION_Y, 0,
...
static int synaptics_rmi4_set_gpio(struct synaptics_rmi4_data *rmi4_data)
{
	...
	/* config interrupt GPIO as input */
	retval = synaptics_rmi4_gpio_setup(bdata->irq_gpio, true, 0, 0);
	if (retval < 0) {
	...
	if (bdata->reset_gpio >= 0) {
		/* config reset GPIO as output, state is inactive */
		retval = synaptics_rmi4_gpio_setup(
				bdata->reset_gpio,
...
/**
 * Enable/Disable F11 wake gesture
 */
...
/**
 * Enable/Disable F12 wake gesture
 */
...
static void synaptics_rmi4_sensor_sleep(struct synaptics_rmi4_data *rmi4_data)
{
	...
	retval = synaptics_rmi4_reg_read(rmi4_data,
			...
			sizeof(device_ctrl));
	if (retval < 0) {
		TP_LOGE("Failed to enter sleep mode\n");
		return;
	}
...
static void synaptics_rmi4_sensor_wake(struct synaptics_rmi4_data *rmi4_data)
{
	...
	retval = synaptics_rmi4_reg_read(rmi4_data,
			...
			sizeof(device_ctrl));
	if (retval < 0) {
		TP_LOGE("Failed to wake from sleep mode\n");
		return;
	}
	...
	if (evdata && evdata->data && rmi4_data) {
		if (event == FB_EVENT_BLANK) {
			...
			if (*transition == FB_BLANK_POWERDOWN) {
				TP_LOGD("fb is blank, do suspend\n");
				synaptics_rmi4_suspend(&rmi4_data->pdev->dev);
				rmi4_data->fb_ready = false;
			} else if (*transition == FB_BLANK_UNBLANK) {
				TP_LOGD("fb is unblank, do resume\n");
...
static void synaptics_rmi4_early_suspend(struct early_suspend *h)
{
	...
	TP_LOGI("Early Suspend - stay_awake(%s), "
			"enable_wakeup_gesture(%s)\n",
			rmi4_data->stay_awake ? "true" : "false",
			rmi4_data->enable_wakeup_gesture ? "true" : "false");
...
static void synaptics_rmi4_late_resume(struct early_suspend *h)
{
	...
	TP_LOGI("Late Resume - stay_awake(%s), "
			"enable_wakeup_gesture(%s), "
			"suspend(%s)\n",
			rmi4_data->stay_awake ? "true" : "false",
			rmi4_data->enable_wakeup_gesture ? "true" : "false",
			rmi4_data->suspend ? "true" : "false");
