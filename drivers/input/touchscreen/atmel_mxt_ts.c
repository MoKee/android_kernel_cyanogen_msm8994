...
/* CEI firmware check */
#define CEI_FW_CHECK_VERSION    0x21
...
static int mxt_debug_msg_init(struct mxt_data *data)
{
	...
	data->debug_msg_attr.attr.mode = 0644;
...
static u8 mxt_get_bootloader_version(struct mxt_data *data, u8 val)
{
	...
	if (val & MXT_BOOT_EXTENDED_ID) {
		...
		TP_LOGD("Bootloader ID:%d Version:%d\n", buf[1], buf[2]);

		return buf[0];
	} else {
		TP_LOGD("Bootloader ID:%d\n", val & MXT_BOOT_ID_MASK);
...
static void mxt_proc_t9_message(struct mxt_data *data, u8 *message)
{
	...
	TP_LOGD("[%u] %c%c%c%c%c%c%c%c x: %5u y: %5u area: %3u amp: %3u vector: %02X\n",
			id,
...
static void mxt_proc_t100_message(struct mxt_data *data, u8 *message)
{
	...
	TP_LOGD("[%u] status:%02X x:%u y:%u area:%02X amp:%02X vec:%02X\n",
			id,
			status,
			x, y,
...
static int mxt_check_retrigen(struct mxt_data *data)
{
	...
	if (irqd_get_trigger_type(irq_get_irq_data(data->irq))
			& IRQF_TRIGGER_LOW)
		return 0;
		...
		size_t config_mem_size)
{
	...
	while (data_pos < cfg->size) {
		...
		if (!object) {
			...
			for (i = 0; i < size; i++) {
				ret = sscanf(cfg->data + data_pos, "%hhx%n",
						&val, &offset);
				if (ret != 1) {
					TP_LOGE("Bad format in T%d at %d\n",
							type, i);
					return -EINVAL;
				}
				data_pos += offset;
		...
		if (size > mxt_obj_size(object)) {
			 ...
			 */
			TP_LOGW("Discarding %zu byte(s) in T%u\n",
			 ...
			 */
			TP_LOGW("Zeroing %zu byte(s) in T%d\n",
...
static int mxt_update_cfg(struct mxt_data *data, const struct firmware *cfg)
{
	...
	TP_LOGI("Config successfully updated\n");
				  ...
				  struct mxt_object *object_table)
{
	...
	for (i = 0; i < data->info->object_num; i++) {
		...
		TP_LOGD("T%u Start:%u Size:%zu Instances:%zu Report IDs:%u-%u\n",
...
static void mxt_regulator_enable(struct mxt_data *data)
{
	...
	if (error) {
		TP_LOGE("Error %d cnnot enable avdd\n", error);
		return;
	}
...
static void mxt_probe_regulators(struct mxt_data *data)
{
	...
	TP_LOGD("Initialized regulators\n");
	return;
...
static int mxt_read_t9_resolution(struct mxt_data *data)
{
	...
	TP_LOGD("Touchscreen size X%uY%u\n", data->max_x, data->max_y);
...
static int mxt_initialize(struct mxt_data *data)
{
	...
	while (1) {
		...
		if (++recovery_attempts > 1) {
			TP_LOGE("Could not recover from bootloader mode\n");
			/*
	...
	if (data->cfg_name) {
		...
		if (error) {
			TP_LOGE("Failed to invoke firmware loader: %d\n",
					error);
			goto err_free_object_table;
		}
	} else {
...
recheck:
	...
	TP_LOGD("Initialized power cfg: ACTV %d, IDLE %d\n",
				 ...
				 const struct firmware *cfg)
{
	...
	if (cfg) {
		...
		if (error)
			TP_LOGW("Error %d updating config\n", error);
	}
...
static int mxt_load_fw(struct device *dev)
{
	...
	while (pos < fw->size) {
		...
		if (frame % 50 == 0)
			TP_LOGD("Sent %d frames, %d/%zd bytes\n",
				frame, pos, fw->size);
	}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (kstrtou8(buf, 0, &i) == 0 && i < 2) {
		...
		TP_LOGD("debug_enabled write error\n");
		ret = -EINVAL;
	}
...
static DEVICE_ATTR(dynamic_log, S_IWUSR | S_IRUGO, mxt_dynamic_log_show,
		mxt_dynamic_log_store);
static DEVICE_ATTR(debug_tp_enable, S_IWUSR, NULL,
		mxt_debug_enable_touch_store);
static DEVICE_ATTR(fw_check, S_IRUGO, mxt_fw_check_show, NULL);
...
static void mxt_stop(struct mxt_data *data)
{
	TP_LOGD("suspended = %s, "
			...
			data->suspended ? "true" : "false",
			data->in_bootloader ? "true" : "false",
			data->updating_config ? "true": "false",
			data->use_regulator ? "true" : "false");
...
static int mxt_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	/* prepare mxt_platform_data */
	pdata = dev_get_platdata(&client->dev);
	if (!pdata) {
		pdata = mxt_parse_dt(client);
		if (IS_ERR(pdata)) {
			TP_LOGE("Parse device tree failed\n");
