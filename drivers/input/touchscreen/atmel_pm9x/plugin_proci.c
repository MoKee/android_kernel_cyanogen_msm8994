...
#define PI_DYNAMIC_LOG

#define PI_LOG_LEVEL_ERROR   1
#define PI_LOG_LEVEL_WARRING 2
#define PI_LOG_LEVEL_INFO    4
#define PI_LOG_LEVEL_DEBUG   8

#ifdef PI_DYNAMIC_LOG
static unsigned int pi_dynamic_log_level = PI_LOG_LEVEL_INFO;

#define pi_dynamic_print(level, fmt, arg...) \
	do { \
		if (level <= pi_dynamic_log_level) \
			printk(fmt, ##arg); \
	} while(0)
#else
#define pi_dynamic_print(level, fmt, arg...) \
	printk(fmt, ##arg);
#endif /* PI_DYNAMIC_LOG */

#define PI_LOGE(fmt, arg...) \
	pi_dynamic_print(PI_LOG_LEVEL_ERROR, \
			"[%s][error] %s: "fmt, PI_LOG_TAG, __func__, ##arg)
#define PI_LOGW(fmt, arg...) \
	pi_dynamic_print(PI_LOG_LEVEL_WARRING, \
			"[%s][warring] %s: "fmt, PI_LOG_TAG, __func__, ##arg)
#define PI_LOGI(fmt, arg...) \
	pi_dynamic_print(PI_LOG_LEVEL_INFO, \
			"[%s][info] %s: "fmt, PI_LOG_TAG, __func__, ##arg)
#define PI_LOGD(fmt, arg...) \
	pi_dynamic_print(PI_LOG_LEVEL_DEBUG, \
			"[%s][debug] %s: "fmt, PI_LOG_TAG, __func__, ##arg)
...
static struct data_obj * get_data_by_element(struct list_head *head,
...
static void plugin_proci_pi_hook_t6(struct plugin_proci *p, u8 status)
{
	...
	if (status & (MXT_T6_STATUS_RESET|MXT_T6_STATUS_CAL)) {
		...
		PI_LOGD("PI hook T6 end\n");
	}

	PI_LOGD("mxt pi flag=0x%lx %x\n", obs->flag, status);
}

/*
static void plugin_cal_pi_hook_t9(struct plugin_proci *p,
		int id, int x, int y, u8 status)
{
}
*/
...
static int plugin_proci_pi_hook_t61(void *pi_id, u8 *msg, unsigned long pl_flag)
{
	...
	PI_LOGI("T61 timer %d status 0x%x %d\n",
			msg[0],
			state,
			(state & MXT_T61_ELAPSED));
...
static int plugin_proci_pi_hook_t81(void *pi_id, u8 *msg,
		unsigned long pl_flag)
{
	...
	PI_LOGI("mxt hook pi t81 0x%x\n", state);

	state &= 0x1;
	if (state) {
		idx = msg[0];  //Instance ID
		PI_LOGI("T81 id %d index %d range %u %u\n",
			msg[0],
			idx,
...
static int plugin_proci_pi_hook_t92(void *pi_id, u8 *msg,
		unsigned long pl_flag)
{
	struct plugin_proci *p = (struct plugin_proci *)pi_id;
...
static int plugin_proci_pi_hook_t93(void *pi_id, u8 *msg,
		unsigned long pl_flag)
{
	struct plugin_proci *p = (struct plugin_proci *)pi_id;
...
static int plugin_proci_pi_hook_t99(void *pi_id, u8 *msg,
		unsigned long pl_flag)
{
	struct plugin_proci *p = (struct plugin_proci *)pi_id;
	...
	PI_LOGI("mxt hook pi t99 Event %d, Key %d\n", state,msg[2]);

	if (state) {
		idx = 0;
		PI_LOGI("T99 key index %d\n", idx);
		clr_gesture_trace_buffer(p);
	}
...
static int get_gesture_trace_data(struct plugin_proci *p, u8 symbol, u8 * buf)
{
	...
	if (p->get_diagnostic_data) {
		...
		do {
			...
			ret = p->get_diagnostic_data(p->dev, MXT_T6_DEBUG_GESTRUE,
					0, 0, 1, buf + GESTURE_TRACE_STROKE_CNT_INDEX, 10, 5);
			if (ret == 0) {
				...
				PI_LOGI("gesture trace data %02x %02x %02x \n",
						buf[0], buf[1], buf[2]);
			}
			msleep(10);
		} while (--retry);
		...
		if (ret == 0) {
			if (strcnt && strcnt < MAX_GESTURE_TRACE_STROKE) {
				PI_LOGI("Found gesture trace data num = %d\n", strcnt);
				ret = p->get_diagnostic_data(p->dev, MXT_T6_DEBUG_GESTRUE,
						0, 1, (strcnt << 2),
						buf + GESTURE_TRACE_STROKE_CNT_INDEX + 1, 10, 5);
			} else {
				PI_LOGE("Invalid gesture trace data num = %d\n", strcnt);
				ret = -EINVAL;
			}
		}
		if (ret == 0) 
			...
			PI_LOGE("Read gesture trace failed %d (%02x %02x %02x)\n",
...
static int set_data_by_element(struct list_head *head,
		const struct ges_tab_element *elem, unsigned long op)
{
	struct list_head *node;
	...
	list_for_each(node, head) {
		...
		if (r->reg == elem->reg &&
			...
			if (test_flag(BIT_MASK(OP_SET), &op) &&
				...
				set_and_clr_flag(
						BIT_MASK(OP_SET),
						BIT_MASK(OP_CLR) | BIT_MASK(GES_ENABLE),
						&r->flag);
			...
			if (test_flag(BIT_MASK(OP_CLR), &op) &&
				...
				set_and_clr_flag(
						BIT_MASK(OP_CLR),
						BIT_MASK(OP_SET) | BIT_MASK(GES_ENABLE),
						&r->flag);
			ret = 0;
			break;
		}
...
static const struct ges_tab_element * get_element_by_data(
		const struct ges_tab_element *tab, int num,
		const struct data_obj *cont)
{
	const struct ges_tab_element *elem;
	int i;
...
static int binding_list_config_set(struct plugin_proci *p,
		const struct reg_config *rset)
{
	struct pi_config *cfg = p->cfg;
	struct reg_config *rcfg;
...
void pi_debug_show_content(struct plugin_proci *p,
		const struct reg_config *r, const char *msg)
{
	PI_LOGI("pi gesture %s obj %d %c(%d) %c%c%c %c%c "
			"(buf %02x, offset %d, len %d, mask 0x%02lx)\n",
			msg ? msg : "",
			r->reg,
			SUBNAME_GES(r->tag) >=
					CHARACTER_ASCII_BEGIN ? SUBNAME_GES(r->tag) : '.',
			SUBNAME_GES(r->tag),
...
static void debug_show_gesture_list(struct plugin_proci *p,
	...
	list_for_each(gnode, ghead) {
		...
		if (!pos || pos == gnode) {
			...
			list_for_each(cnode, chead) {
				...
				r = &cont->rcfg;
				PI_LOGI("\t sub gesture offset %d,len %d,mask 0x%02lx\n",
...
static int plugin_proci_pi_handle_gesture(struct plugin_proci *p,
		unsigned long op, bool reset)
{
	struct pi_config *cfg = p->cfg;
	...
	PI_LOGD("pi gesture handle op  %c%c%c (%lx) reset %d\n",
		...
		if (test_flag(BIT_MASK(GES_ENABLE), &op)) {
			...
			if (test_flag(BIT_MASK(GES_CTRL_EN), &r->tag)) {
				...
				if (op2) {
					...
					if (ret == 0) {
						...
						PI_LOGE("pi gesture write reg %d off %d len %d failed\n",
			...
			if (!list_empty(chead) && 
				...
				list_for_each(cnode, chead) {
					...
					ret = p->set_obj_cfg(p->dev, &rcfg, NULL, 0);
					if (ret == -EIO) {
						PI_LOGE("pi ctrl write reg %d off %d len %d failed\n",
								rcfg.reg, rcfg.offset, rcfg.len);
						break;
					} else
						ret = 0;
...
static int pi_handle_request(struct plugin_proci *p,
		const struct reg_config *rcfg, int num_reg_cfg,
		struct reg_config *rset, struct reg_config *save, unsigned long flag)
{
	struct pi_observer *obs = (struct pi_observer *)p->obs;
	...
	for (i = 0; i < num_reg_cfg; i++) {
		...
		*/
		...
		if (ret == -EIO) {
			PI_LOGE("pi request write reg %d off %d len %d failed\n",
...
static int plugin_proci_pi_handle_request(struct plugin_proci *p,
		int pi, bool enable, unsigned long pl_flag)
{
	struct pi_observer *obs = p->obs;
	struct pi_config *cfg = p->cfg;
	...
	if (enable) {
		...
		if (ret) {
			PI_LOGE("pi handle request write reg %d offset %d len %d failed\n",
	...
	//restore register
		...
		ret = pi_handle_request(p, rstack, cfg->num_reg_cfg[pi],
				rset, /*rstack*/NULL, flag);
		if (ret) {
			PI_LOGE("pi handle request write reg %d offset %d len %d failed\n",
					rset->reg, rset->offset, rset->len);
			mutex_unlock(&obs->access_mutex);
	...
	PI_LOGI("pi handle request result %d\n", ret);

	mutex_unlock(&obs->access_mutex);
	return ret;
}

static int plugin_proci_pi_wakeup_enable(struct plugin_proci *p,
		unsigned long pl_flag)
{
	struct pi_observer *obs = p->obs;
	...
	if (test_flag(PI_FLAG_WAKEUP, &obs->flag)) {//some error occur in suspend, reset chip
		PI_LOGE("set wakeup enable loop, set reset\n");
		if (p->reset)
			ret = p->reset(p->dev, 0);
		if (ret == 0)
	...
	if (!test_flag(PI_FLAG_WAKEUP, &obs->flag)) {
		//clr_gesture_trace_buffer(p);
		ret = plugin_proci_pi_handle_gesture(p,
				BIT_MASK(GES_ENABLE) | BIT_MASK(OP_SET), 0);
		if (ret == 0){
		...
		if (ret) {
			PI_LOGE("set wakeup enable failed, set reset\n");
			if (p->reset)
				ret = p->reset(p->dev, 0);
			if (ret != 0)
				p->set_and_clr_flag(p->dev, PL_STATUS_FLAG_NEED_RESET, 0);
...
static int plugin_proci_pi_wakeup_disable(struct plugin_proci *p,
		unsigned long pl_flag)
{
	struct pi_observer *obs = p->obs;
	...
	if (test_flag(PI_FLAG_WAKEUP, &obs->flag)) {
		if (plugin_proci_pi_handle_gesture(
				p, BIT_MASK(GES_ENABLE) | BIT_MASK(OP_CLR), 0) == 0) {
			if (plugin_proci_pi_handle_request(
					p, PI_DWAKE, false, pl_flag) == 0)
				clear_flag(PI_FLAG_WAKEUP, &obs->flag);
		}
	}

	if (test_flag(PI_FLAG_WAKEUP, &obs->flag)) {
		PI_LOGE("set wakeup disable failed, set reset\n");
		if (p->reset)
			ret = p->reset(p->dev, 0);
		if (ret == 0)
			clear_flag(PI_FLAG_WAKEUP, &obs->flag);
...
int gesture_switch;
#endif
static void plugin_proci_pi_pre_process_messages(struct plugin_proci *p,
		unsigned long pl_flag)
{
	//struct pi_config *cfg = p->cfg;
	struct pi_observer *obs = p->obs;
	...
	PI_LOGD("mxt plugin_proci_pi_pre_process_messages "
			"pl_flag=0x%lx flag=0x%lx\n",
			pl_flag, obs->flag);
...
static long plugin_proci_pi_post_process_messages(struct plugin_proci *p,
		unsigned long pl_flag)
{
	struct pi_observer *obs = p->obs;
	bool enable;
	...
	struct pi_descriptor dpr_ges = {
		PL_FUNCTION_FLAG_WAKEUP_GESTURE,
		PI_FLAG_GESTURE
	};
	...
	for (i = 0; i < ARRAY_SIZE(dpr); i++) {
		if (test_flag(dpr[i].flag_pl,&pl_flag) !=
				test_flag(dpr[i].flag_pi, &obs->flag)){

			enable = test_flag(dpr[i].flag_pl, &pl_flag);
			ret = plugin_proci_pi_handle_request(p, dpr[i].pi, enable, pl_flag);
			if (ret) {
				PI_LOGE("set pi %d enable %d failed %d\n",
						dpr[i].pi, enable, ret);
				if (p->reset)
					ret = p->reset(p->dev, 0);
				if (ret == 0)
	...
	if (test_flag(dpr_ges.flag_pl, &pl_flag) !=
			test_flag(dpr_ges.flag_pi, &obs->flag)) {
		plugin_proci_pi_handle_gesture(
				p, BIT_MASK(GES_ENABLE) | BIT_MASK(OP_CLR), false); //set all gesture control bit disabled
		plugin_proci_pi_handle_gesture(
				p, 0, test_flag(PI_FLAG_RESET, &obs->flag)); //update all gesture sublist
...
static const struct ges_tab_element gesture_element_array[] = {
	...
	{
		MXT_PROCI_KEYGESTUREPROCESSOR_T99,
		0,
		BIT_MASK(GES_CTRL_EN),
		"KEY_TAP"
	},
	{
		...
		0,
		UNLOCK_0 | BIT_MASK(GES_CTRL_EN),
		"UNLOCK0"
	},
	{
		MXT_PROCI_UNLOCKGESTURE_T81,
		1,
		UNLOCK_1 | BIT_MASK(GES_CTRL_EN),
		"UNLOCK1"
	},
	{
	...
	},
	{
		MXT_PROCI_SYMBOLGESTURE_T115,
		0,
		SLIDING_UP | BIT_MASK(GES_CTRL_EN) | BIT_MASK(GES_SWITCH),
		"UP"
	},
	{
		MXT_PROCI_SYMBOLGESTURE_T115,
...
ssize_t plugin_proci_pi_gesture_show(struct plugin_proci *p,
		char *buf, size_t count)
{
	...
	list_for_each(node, head) {
		...
		elem = get_element_by_data(
				gesture_element_array,
...
int plugin_proci_pi_gesture_store(struct plugin_proci *p,
		const char *buf, size_t count)
{
	...
	if (check_switch) {
		...
		if (set_data_by_element(head, &element, op) != 0) {
			PI_LOGE("Failed process T115 SLIDING_AND_CHARACTER\n");
			ret = -EINVAL;
		}
	}
...
ssize_t plugin_proci_pi_trace_show(struct plugin_proci *p,
		char *buf, size_t count)
{
	struct pi_observer *obs = p->obs;
	int offset,strcnt;
...
static int plugin_proci_pi_show(struct plugin_proci *p)
{
	...
	PI_LOGI("PLUG_PROCI_VERSION: 0x%x\n", PLUG_PROCI_VERSION);

	if (!p->init)
		return 0;

	PI_LOGI("pi status: Flag=0x%08lx\n", obs->flag);
	...
	for (i = 0; i < PI_LIST_NUM; i++) {
		...
		PI_LOGI("PI config %d '%s':\n", i, pi_cfg_name[i]);

		PI_LOGI("rcfg %d\n", cfg->num_reg_cfg[i]);
		r = rcfg;
		for (j = 0; j < cfg->num_reg_cfg[i]; j++) {
			PI_LOGI("config %d-%d: T%d(%d) offset %d len %d(%lx, %lx, %hhd):\n",
					i, j,
					r[j].reg,
					r[j].instance,
					r[j].offset,
			...
			print_hex_dump(KERN_INFO, "[atmel]", DUMP_PREFIX_NONE, 16, 1,
		...
		r = rset;
		for (j = 0; j < cfg->num_reg_cfg[i]; j++) {
			PI_LOGI("config %d-%d: T%d offset %d len %d(%lx,%lx): ",
					i,j, r[j].reg,r[j].offset,r[j].len,r[j].mask,r[j].flag);
...
static int plugin_proci_pi_store(struct plugin_proci *p,
		const char *buf, size_t count)
{
	struct pi_observer * obs = p->obs;
	struct pi_config *cfg = p->cfg;
	...
	if (sscanf(buf, "status: Flag=0x%lx\n",
		...
		if (count > 6 && count < sizeof(name)) {
			...
			PI_LOGI("name %s, offset %d, ret %d\n", name, offset, ret);
			if (ret == 1) {
				if (strncmp(name, "config", 6) == 0) {
					ret = sscanf(buf + offset, "%d-%d: %n", &i, &j, &ofs);
					PI_LOGI("config (%d,%d), offset %d ret %d^\n", i, j, ofs, ret);
					...
					if (ret == 2) {
						...
						if (i >= 0 && i < PI_LIST_NUM) {
							if (j >= 0 && j < cfg->num_reg_cfg[i]) {
								...
								ofs = 0;
								ret = sscanf(buf + offset,
										"T%hd(%hd) offset %hd len %hd(%lx,%lx,%hhd): %n",
										&rc.reg,
										&rc.instance,
										&rc.offset,
										&rc.len,
								...
								if (ret > 0) {
									PI_LOGI("%s\n", buf + offset);
									PI_LOGI("T%hd(%d) offset %hd len %hd(%lx,%lx):(ret %d ofs %d)^",
											rc.reg,
											rc.instance,
											rc.offset,
											rc.len,
									...
									if (k && ret > 0) {
										...
										print_hex_dump(KERN_INFO,
												...
												16, 1,
												rc.buf, k, false);
										PI_LOGI("set buf data %d\n", k);
									}
									
									memcpy(&r[j], &rc, sizeof(struct reg_config));
								} else {
									PI_LOGI("invalid string: %s\n",
											buf + offset);
								}
							}
						}
					...
					if (ret > 0) {
						PI_LOGI("%s\n", buf + offset);
						PI_LOGD("T%hd offset %hd len %hd:(ret %d)^",
								rc.reg,
								rc.offset,
								rc.len,
								ret);
					...
					if (ret == 1) {
						...
						if (ret == 2) {
							ghead = &cfg->gesture_list;
							element.tag = ges;
							ret = set_data_by_element(
									ghead,
									&element,
									val ? BIT_MASK(OP_SET) : BIT_MASK(OP_CLR));
							if (ret != 0) {
								PI_LOGE("Unknow command(T): %s\n", buf);
								return -EINVAL;
							}
						} else {
							PI_LOGE("Uncompleted command(T%d %d): %s\n",
									SUBNAME_GES(element.tag),
									val,
									buf);
							return -EINVAL;
						}
					...
					PI_LOGE("Unknow command: %s\n", buf);
					return -EINVAL;
				}
			} else {
				PI_LOGE("Unknow parameter, ret %d\n", ret);
			}
		}
	}
...
static void plugin_proci_pi_dynamic_log(u8 level)
{
	...
	for (i = 0; i < ARRAY_SIZE(gesture_element_array); i++) {
		...
		if (ret == 0) {
			gobj = (struct gesture_obj *)create_new_node(
					&cfg->gesture_list, sizeof(struct gesture_obj));
			if (!gobj) {
				PI_LOGE("Failed to allocate memory for T%d gesture list\n",
						rcfg.reg);
				ret = -ENOMEM;
				goto release;
			}
		...
		if (ret == 0) {
				...
				if (sym_cnt) {
					if (sym_tag >= CHARACTER_ASCII_BEGIN) {
						...
						if (!cont) {
							PI_LOGE("Failed to allocate memory for T%d ctrl %d list\n",
									rcfg.reg, i);
							ret = -ENOMEM;
							goto release;
						}
...
static int init_pi_object(struct plugin_proci *p)
{
	...
	PI_LOGI("alloc mem %d, each %lu\n",
			mem_size, sizeof(struct reg_config));
	
	obs->mem = kzalloc(mem_size, GFP_KERNEL);
	if (!obs->mem) {
		PI_LOGE("Failed to allocate memory for pi observer reg mem\n");
		return -ENOMEM;
	}
	...
	if (ret) {
		PI_LOGE("Failed to call init_gesture_list\n");
	}
...
{
	//struct plug_interfwakee *pl =
	//		container_of(dcfg, struct plug_interfwakee, init_cfg);

	PI_LOGI("plugin proci pi version 0x%x\n", PLUG_PROCI_VERSION);

	p->obs = kzalloc(sizeof(struct pi_observer), GFP_KERNEL);
	if (!p->obs) {
		PI_LOGE("Failed to allocate memory for pi observer\n");
		return -ENOMEM;
	}

	p->cfg = kzalloc(sizeof(struct pi_config), GFP_KERNEL);
	if (!p->cfg) {
		PI_LOGE("Failed to allocate memory for pi cfg\n");
		kfree(p->obs);
		p->obs = NULL;
		return -ENOMEM;
	}
...
static struct plugin_proci mxt_plugin_proci_pi_if = {
	...
	.dynamic_log = plugin_proci_pi_dynamic_log,
};
