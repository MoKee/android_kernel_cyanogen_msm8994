...
/* Log */
#define WD_LOG_TAG "atmel WD"

#define WD_DYNAMIC_LOG

#define WD_LOG_LEVEL_ERROR   1
#define WD_LOG_LEVEL_WARRING 2
#define WD_LOG_LEVEL_INFO    4
#define WD_LOG_LEVEL_DEBUG   8

#ifdef WD_DYNAMIC_LOG
static unsigned int wd_dynamic_log_level = WD_LOG_LEVEL_INFO;

#define wd_dynamic_print(level, fmt, arg...) \
	do { \
		if (level <= wd_dynamic_log_level) \
			printk(fmt, ##arg); \
	} while(0)
#else
#define wd_dynamic_print(level, fmt, arg...) \
	printk(fmt, ##arg);
#endif /* WD_DYNAMIC_LOG */

#define WD_LOGE(fmt, arg...) \
	wd_dynamic_print(WD_LOG_LEVEL_ERROR, \
			"[%s][error] %s: "fmt, WD_LOG_TAG, __func__, ##arg)
#define WD_LOGW(fmt, arg...) \
	wd_dynamic_print(WD_LOG_LEVEL_WARRING, \
			"[%s][warring] %s: "fmt, WD_LOG_TAG, __func__, ##arg)
#define WD_LOGI(fmt, arg...) \
	wd_dynamic_print(WD_LOG_LEVEL_INFO, \
			"[%s][info] %s: "fmt, WD_LOG_TAG, __func__, ##arg)
#define WD_LOGD(fmt, arg...) \
	wd_dynamic_print(WD_LOG_LEVEL_DEBUG, \
			"[%s][debug] %s: "fmt, WD_LOG_TAG, __func__, ##arg)
	...
	if (test_flag(WD_FLAG_REG_OVERFLOW ,&flag)) {
		...
		if (re_obs->curr >= re_cfg->failed_reset_count) {
			if (re_obs->retry < re_cfg->failed_reset_retry) {
				WD_LOGI("reg set reset count %d(%d) retry %d(%d)\n",
						re_obs->curr,
						re_cfg->failed_reset_count,
						re_obs->retry,
						re_cfg->failed_reset_retry);
				if (p->reset)
...
static long wdg_handle_ti_event(struct plugin_wdg *p, unsigned long flag)
{
	...
	if (test_flag(WD_FLAG_RESET, &flag)) {
		...
		WD_LOGD("WD hook T61 check overflow\n");

		if (ti_obs->count >= ti_cfg->min_check_count) {
			if (time_after_eq(jiffies,
					ti_obs->time_wd_check_point + ti_cfg->interval_wd_check)) {
				WD_LOGI("mxt WD time out jiffise(%ld) check(%ld %ld) "
						"count %d(%d) retry %d(%d)\n",
						jiffies,
						ti_obs->time_wd_check_point,
						ti_cfg->interval_wd_check,
						ti_obs->count,
						ti_cfg->min_check_count,
						ti_obs->retry,
						ti_cfg->failed_reset_retry);
				ti_obs->failed++;
...
static void plugin_wdg_hook_t6(struct plugin_wdg *p, u8 status)
{
	...
	if (status & (MXT_T6_STATUS_RESET | MXT_T6_STATUS_CAL)) {
		...
		WD_LOGD("WD hook T6 end\n");
	}

	WD_LOGD("mxt wd flag=0x%lx %x\n", obs->flag, status);
}
...
static void plugin_wdg_hook_t61(struct plugin_wdg *p, int id, u8 state)
{
	...
	WD_LOGD("mxt wd timer(%d) state %x\n", id, state);
...
static int plugin_wdg_hook_reg_access(struct plugin_wdg *p,
		u16 addr, u16 reg, u16 len, const void *val,
		unsigned long flag, int result, bool is_w)
{
	struct wd_observer *obs = p->obs;
	int ret = result;
...
static void plugin_wdg_pre_process_messages(struct plugin_wdg *p,
		unsigned long pl_flag)
{
	...
	WD_LOGD("mxt plugin_wdg_pre_process_messages pl_flag=0x%lx flag=0x%lx\n",
			pl_flag, obs->flag);
}

static long plugin_wdg_post_process_messages(struct plugin_wdg *p,
		unsigned long pl_flag)
{
	struct wd_config *cfg = p->cfg;
	struct wd_observer *obs = p->obs;
	...
	WD_LOGD("mxt wd pl_flag=0x%lx flag=0x%lx\n",
			pl_flag, obs->flag);
	...
	if (test_flag(WD_FLAG_FUNC_TIMER, &obs->flag)) 
		ret[1] = wdg_handle_ti_event(
				p,
				test_flag(WD_FLAG_RESET, &obs->flag) ?
...
static void plugin_wdg_start(struct plugin_wdg *p, bool resume)
{
	...
	if (test_flag(WD_FLAG_TIMER_OVERFLOW | WD_FLAG_REG_OVERFLOW, &obs->flag)) {
		WD_LOGE("wdg start reset flag 0x%lx\n", obs->flag);
		if (p->reset)
			ret = p->reset(p->dev, 1);
		if (ret == 0)
...
static void plugin_wdg_stop(struct plugin_wdg *p)
{
	...
	if (test_flag(WD_FLAG_TIMER_OVERFLOW | WD_FLAG_REG_OVERFLOW, &obs->flag)) {
		WD_LOGE("wdg stop reset flag 0x%lx\n", obs->flag);
		if (p->reset)
			ret = p->reset(p->dev, 1);
		if (ret == 0)
			clear_flag(WD_FLAG_TIMER_OVERFLOW | WD_FLAG_REG_OVERFLOW, &obs->flag);
...
ssize_t plugin_wdg_show(struct plugin_wdg *p, char *buf, size_t count)
{
	...
	WD_LOGI("wdg status: Flag=0x%08lx\n",
			obs->flag);

	WD_LOGI("wdg ti: interval %lx count %d retry %d\n",
			ti_cfg->interval_wd_check,
			ti_cfg->min_check_count,
			ti_cfg->failed_reset_retry);

	WD_LOGI("wdg re: count %d retry %d\n",
			re_cfg->failed_reset_count,
			re_cfg->failed_reset_retry);

	WD_LOGI("wdg ti: st %lx count %d failed %d retry %d flag %lx\n",
			ti_obs->time_wd_check_point,
			ti_obs->count,
			ti_obs->failed,
			ti_obs->retry,
	...
	WD_LOGI("wdg re: failed %d count %d retry %d flag %lx\n",
			re_obs->failed,
			re_obs->curr,
			re_obs->retry,
			re_obs->flag);
			...
			if (ret == 1) {
				if (strncmp(name, "ti", 2) == 0) {
					if (sscanf(buf + offset, "interval %lx count %d retry %d\n",
						...
						WD_LOGE("Unknow wd ti command: %s\n", buf + offset);
					}
				} if (strncmp(name, "re", 2) == 0) {
					if (sscanf(buf + offset, "re: count %d retry %d\n",
							&re_cfg->failed_reset_count,
							&re_cfg->failed_reset_retry) == 2) {
					} else {
						WD_LOGE("Unknow wd ti command: %s\n", buf + offset);
					}
				} else {
					WD_LOGE("Unknow wd command: %s\n", buf);
					return -EINVAL;
				}
			} else {
				WD_LOGE("Unknow parameter, ret %d\n", ret);
			}
		}
	}
...
static int init_wd_object(struct plugin_wdg *p)
{
	...
	ret = init_ti(p);
	if (ret) {
		WD_LOGE("Failed to init_ti\n");
		return -ENOMEM;
	}

	ret = init_re(p);
	if (ret) {
		WD_LOGE("Failed to init_re\n");
		return -ENOMEM;
	}
...
{
	WD_LOGI("plugin wdg wd version 0x%x\n", PLUG_WDG_VERSION);

	p->obs = kzalloc(sizeof(struct wd_observer), GFP_KERNEL);
	if (!p->obs) {
		WD_LOGE("Failed to allocate memory for wd observer\n");
		return -ENOMEM;
	}

	p->cfg = kzalloc(sizeof(struct wd_config), GFP_KERNEL);
	if (!p->cfg) {
		WD_LOGE("Failed to allocate memory for wd cfg\n");
		kfree(p->obs);
		p->obs = NULL;
		return -ENOMEM;
	}
