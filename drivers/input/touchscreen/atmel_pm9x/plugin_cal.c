...
/* Log */
#define CR_LOG_TAG "atmel CR"

#define CR_DYNAMIC_LOG

#define CR_LOG_LEVEL_ERROR   1
#define CR_LOG_LEVEL_WARRING 2
#define CR_LOG_LEVEL_INFO    4
#define CR_LOG_LEVEL_DEBUG   8

#ifdef CR_DYNAMIC_LOG
static unsigned int cr_dynamic_log_level = CR_LOG_LEVEL_INFO;

#define cr_dynamic_print(level, fmt, arg...) \
	do { \
		if (level <= cr_dynamic_log_level) \
			printk(fmt, ##arg); \
	} while(0)
#else
#define cr_dynamic_print(level, fmt, arg...) \
	printk(fmt, ##arg);
#endif /* CR_DYNAMIC_LOG */

#define CR_LOGE(fmt, arg...) \
	cr_dynamic_print(CR_LOG_LEVEL_ERROR, \
			"[%s][error] %s: "fmt, CR_LOG_TAG, __func__, ##arg)
#define CR_LOGW(fmt, arg...) \
	cr_dynamic_print(CR_LOG_LEVEL_WARRING, \
			"[%s][warring] %s: "fmt, CR_LOG_TAG, __func__, ##arg)
#define CR_LOGI(fmt, arg...) \
	cr_dynamic_print(CR_LOG_LEVEL_INFO, \
			"[%s][info] %s: "fmt, CR_LOG_TAG, __func__, ##arg)
#define CR_LOGD(fmt, arg...) \
	cr_dynamic_print(CR_LOG_LEVEL_DEBUG, \
			"[%s][debug] %s: "fmt, CR_LOG_TAG, __func__, ##arg)
...
static struct samples *find_next_sample(struct list_head *head,
		int check_sign, unsigned long ticks);

static ssize_t rc_mv_show(struct plugin_cal *p,
...
static void plugin_cal_hook_t6(struct plugin_cal *p, u8 status)
{
	...
	if (status & (MXT_T6_STATUS_RESET|MXT_T6_STATUS_CAL)) {
		...
		CR_LOGD("CR hook T6 end\n");
	}

	CR_LOGD("mxt rc flag=0x%lx %x\n", obs->flag, status);
}
...
static void rc_mv_hook_t9_t100_points(struct plugin_cal *p,
		int id, int x, int y, unsigned long flag, unsigned long ticks)
{
	...
	if (test_flag(RC_ACTION_POINT_FIRST_TOUCH | RC_ACTION_POINT_NEXT_TOUCH, &flag)) {
		do {
			...
			if (!pt->cnt) {
				...
				pt->ticks = ticks;
				pt->cnt = 1;
				CR_LOGD("point (%d %d): (%d,%d) cnt %d time %ld #1\n",
					id, idx,
					pt->pos.x / pt->cnt,
					pt->pos.y / pt->cnt,
					pt->cnt, jiffies -pt->ticks);
			} else {
				if (time_after(ticks, pt->ticks + mv_cfg->interval_sampling)) {
					...
					pt->pos.y += y;
					pt->cnt++;
					CR_LOGD("point (%d %d): (%d,%d) cnt %d time %ld #2\n",
						id, idx,
						pt->pos.x / pt->cnt,
						pt->pos.y / pt->cnt,
						pt->cnt, jiffies -pt->ticks);
...
static void rc_se_hook_t9_t100_points(struct plugin_cal *p,
		int id, int x, int y, unsigned long flag)
{
	struct rc_config *cfg = p->cfg;
	struct rc_observer *obs = p->obs;
...
static void rc_sc_hook_t9_t100_points(struct plugin_cal *p,
		int id, int x, int y, unsigned long flag)
{
	struct rc_config *cfg = p->cfg;
	struct rc_observer *obs = p->obs;
...
static int plugin_cal_hook_t9_t100(struct plugin_cal *p,
		int id, int x, int y, struct ext_info *in)
{
	struct rc_observer *obs = p->obs;
	...
	CR_LOGD("tch status %x\n", in->status);

	return 0;
}

static int plugin_cal_hook_t9_t100_scraux(struct plugin_cal *p,
		struct scr_info *in)
{
	return 0;
}
...
static void rc_sampling_recovery(struct plugin_cal *p,
		int check_sign, unsigned long ticks)
{
	struct rc_config *cfg = p->cfg;
	struct rc_observer *obs = p->obs;
...
static int rc_handle_mv_message(struct plugin_cal *p)
{
	...
	for (i = 0; i <MAX_TRACE_POINTS; i++) {	
		...
		for (j = 0; j < (tra->curr & POINT_CACHE_MASK); j++) {
			...
			CR_LOGD("point (%d %d): (%d,%d) cnt %d time %ld #3\n",
					i, k,
					pt->pos.x / pt->cnt,
					pt->pos.y / pt->cnt,
					pt->cnt,
					jiffies - (pt->ticks + mv_cfg->interval_valid));
			...
			if (time_before(jiffies, pt->ticks + mv_cfg->interval_valid)) {
				...
				CR_LOGI("trace(%d, %d) (%d %d)-(%d %d) mov %d %d time %ld\n",
						i,j,
						src0.x, src0.y,
						src1.x, src1.y,
						dst0, dst1,
...
static int rc_handle_se_message(struct plugin_cal *p, const struct samples *s)
{
	...
	CR_LOGI("rc_handle_se_message se (%d %d %d) bad %d pend %d sleep %d\n",
			se_dbg->se_cal_metric,
			se_dbg->se_cal_cnt,
			se_dbg->se_cal_state,
			se_obs->cnt.bad,
			se_obs->cnt.pend,
...
static int rc_handle_sc_message(struct plugin_cal *p, const struct samples *s)
{
	...
	for (i = 0; i < channel; i++) {
		...
		} else if(s->buf[i] >= -condi[SC_NO].thld &&
				s->buf[i] <= condi[SC_NO].thld)
			val[SC_NO]++;
	...
	if (val[SC_POS] >= condi[SC_POS].num &&
			val[SC_ACCU] > condi[SC_ACCU_K].thld) {
		sc_obs->cnt.bad = 0;
		sc_obs->cnt.pend = 0;
		sc_obs->cnt.retry = 0;
	} else {
	...
	//if (ret) {
	CR_LOGD("rc_handle_sc_message sc(%ld) cnt (accu %d neg %d nor %d pos %d) "
			"count %d bad %d pend %d sleep %d retry %d touch(%lx)\n",
			s->type,
			val[SC_ACCU], val[SC_NEG],	val[SC_NO],val[SC_POS],
			sc_obs->cnt.count,
			sc_obs->cnt.bad,
			sc_obs->cnt.pend,
			sc_obs->cnt.sleep,
...
static int rc_handle_sleep(struct plugin_cal *p)
{
	...
	CR_LOGD("rc_handle_sleep sleep(%d,%d) ret %d\n",
...
int rc_aquire_samples(struct plugin_cal *p, struct samples *s, bool send)
{
	...
	CR_LOGD("surface aquire sc (cmd %d, page %d, index %d, num %d) "
			"step(%d %d %d)interval(%d %d)\n",
			cmd_block.diag.cmd,
			cmd_block.diag.page,
			cmd_block.diag.index,
			cmd_block.diag.num,
			cmd_block.curr,
			cmd_block.step,
			cmd_block.max_step,
...
static int rc_cal_handler(struct plugin_cal *p, const struct samples *s)
{
	...
	if (ret == -EINVAL) {
		CR_LOGI("mxt rc_cal_handler set cal: se(%d %d) sc(%d %d)\n",
...
static void plugin_cal_pre_process_messages(struct plugin_cal *p,
		unsigned long pl_flag)
{
	...
	CR_LOGD("mxt plugin_cal_pre_process_messages pl_flag=0x%lx flag=0x%lx\n",
			pl_flag, obs->flag);
}

static long plugin_cal_post_process_messages(struct plugin_cal *p,
		unsigned long pl_flag)
{
	struct rc_config *cfg = p->cfg;
	struct rc_observer *obs = p->obs;
	...
	CR_LOGD("mxt rc pl_flag=0x%lx flag=0x%lx\n",
			pl_flag, obs->flag);
	...
	if (s) {
		CR_LOGD("mxt s type %lx cnt %d curr %ds\n", s->type, s->cnt, s->curr);
		WARN_ON(s->type >= NUM_OBJ_TYPE);
		WARN_ON(!s->cnt);
...
static ssize_t rc_mv_show(struct plugin_cal *p, char *buf, size_t count)
{
	...
	CR_LOGI("mv status: Flag=0x%lx\n", mv_obs->flag);

	CR_LOGI("mv distance: %d\n", mv_cfg->distance);

	CR_LOGI("mv interval: %ld %ld\n",
	...
	for (i = 0; i < MAX_TRACE_POINTS; i++) {
		...
		CR_LOGI("trace %d curr %d distance %d\n",
		...
		for (j = 0; j < POINT_CACHE; j++) {
			...
			CR_LOGI("point %d: (%d,%d) cnt %d time %ld\n",
					j,
					pt->pos.x / pt->cnt,
					pt->pos.y / pt->cnt,
					pt->cnt,
...
static ssize_t rc_se_show(struct plugin_cal *p, char *buf, size_t count)
{
	...
	CR_LOGI("se status: Flag=0x%08lx\n", se_obs->flag);

	if (count > 0) {
		offset += scnprintf(buf + offset,
				count - offset,
				"SE CFG: thld %d hysis %d bad %d pend %d sleep %d\n",
				se_cfg->thld,
				se_cfg->hysis,
				se_cfg->count.bad,
				se_cfg->count.pend,
...
ssize_t plugin_cal_show(struct plugin_cal *p, char *buf, size_t count)
{
	...
	CR_LOGI("cal status: Flag=0x%08lx\n", obs->flag);

	CR_LOGI("cal touch list %lx\n", obs->touch_id_list);
		...
		if (count > 4 && count < sizeof(name)) {
			...
			if (ret == 1) {
				if (strncmp(name, "rc", 2) == 0) {
					if (sscanf(buf + offset,
					...
					} else if (sscanf(buf + offset,
							"interval_ticks[%d]: %d%n",
							&config[0],
							&config[1],&ofs) == 2) {
						offset += ofs;
						if (config[0] < NUM_OBJ_TYPE)
						...
						CR_LOGE("Unknow rc sc command: %s\n", buf + offset);
					}
					...
					if (sscanf(buf + offset,
						...
						CR_LOGE("Unknow rc mv command: %s\n", buf + offset);
					}	
					...
					CR_LOGE("Unknow rc command: %s\n", buf);
					return -EINVAL;
				}
			} else {
				CR_LOGE("Unknow parameter, ret %d\n", ret);
			}
		}
	}
...
static int init_cal_obj_list(struct plugin_cal *p,
		struct list_head *head, int obj_size)
{
	const struct mxt_config *dcfg = p->dcfg;
	struct rc_observer * obs = p->obs;
	struct sampling_obj *sobj;
	...
	if (!buf) {
		CR_LOGE("Failed to allocate memory\n");
		ret = -ENOMEM;
		goto release;
	}
	CR_LOGI("allocated CAL buffer at %p, node number %d\n", buf,size);
	obs->obs_buf = buf;

	//SE
	sobj = (struct sampling_obj *)create_new_node(head, sizeof(struct sampling_obj));
	if (!sobj) {
		CR_LOGE("Failed to allocate memory for SE sampling list\n");
		ret = -ENOMEM;
		goto release;
	}
	...
	if (!cobj) {
		CR_LOGE("Failed to allocate memory for SE list\n");
		ret = -ENOMEM;
		goto release;
	}
	cobj->cont.type = SE;
	...
	sobj = (struct sampling_obj *)create_new_node(head, sizeof(struct sampling_obj));
	if (!sobj) {
		CR_LOGE("Failed to allocate memory for SC sampling list\n");
		ret = -ENOMEM;
		goto release;
	}
	INIT_LIST_HEAD(&sobj->sublist);
	...
	for (i = SC_TOUCH; i < NUM_OBJ_TYPE; i++) {
		cobj = (struct content_obj *)create_new_node(&sobj->sublist, sizeof(struct content_obj));
		if (!cobj) {
			CR_LOGE("Failed to allocate memory for SC %d list\n", i);
			ret = -ENOMEM;
			goto release;
		}
		cobj->cont.type = i;
...
static int init_rc_object(struct plugin_cal *p)
{
	...
	ret = init_cal_obj_list(p, &obs->sampling_list,
			dcfg->s[SC].x + dcfg->s[SC].y);
	if (ret) {
		CR_LOGE("Failed to init_cal_obj_list\n");
		return -ENOMEM;
	}

	ret = init_mov(p);
	if (ret) {
		CR_LOGE("Failed to init_mov\n");
		return -ENOMEM;
	}

	ret = init_se(p);
	if (ret) {
		CR_LOGE("Failed to init_se\n");
		return -ENOMEM;
	}

	ret = init_sc(p);
	if (ret) {
		CR_LOGE("Failed to init_sc\n");
		return -ENOMEM;
	}
...
{
	CR_LOGI("plugin cal version 0x%x\n", PLUG_CAL_VERSION);

	p->obs = kzalloc(sizeof(struct rc_observer), GFP_KERNEL);
	if (!p->obs) {
		CR_LOGE("Failed to allocate memory for rc observer\n");
		return -ENOMEM;
	}

	p->cfg = kzalloc(sizeof(struct rc_config), GFP_KERNEL);
	if (!p->cfg) {
		CR_LOGE("Failed to allocate memory for rc cfg\n");
		kfree(p->obs);
		p->obs = NULL;
		return -ENOMEM;
	}
