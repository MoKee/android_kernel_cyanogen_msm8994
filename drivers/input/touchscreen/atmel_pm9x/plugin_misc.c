...
static void plugin_misc_hook_t6(struct plugin_misc *p, u8 status)
{
	...
	dev_dbg2(dev, "mxt msc flag=0x%lx %x\n", obs->flag, status);
}
...
/*
static void plugin_misc_hook_reg_init(struct plugin_misc *p)
{
	const struct mxt_config *dcfg = p->dcfg;
	struct device *dev = dcfg->dev;
	struct msc_observer *obs = p->obs;
 
	dev_info2(dev, "mxt hook pi reg init\n");

	clear_flag(MSC_FLAG_PTC_PARA_VALID, &obs->flag);
}
*/
