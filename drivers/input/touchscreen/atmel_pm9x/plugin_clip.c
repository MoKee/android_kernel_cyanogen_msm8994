...
/* Log */
#define CLP_LOG_TAG "atmel CLP"

#define CLP_DYNAMIC_LOG

#define CLP_LOG_LEVEL_ERROR   1
#define CLP_LOG_LEVEL_WARRING 2
#define CLP_LOG_LEVEL_INFO    4
#define CLP_LOG_LEVEL_DEBUG   8

#ifdef CLP_DYNAMIC_LOG
static unsigned int clp_dynamic_log_level = CLP_LOG_LEVEL_INFO;

#define clp_dynamic_print(level, fmt, arg...) \
	do { \
		if (level <= clp_dynamic_log_level) \
			printk(fmt, ##arg); \
	} while(0)
#else
#define clp_dynamic_print(level, fmt, arg...) \
...
#define CLP_LOGE(fmt, arg...) \
	clp_dynamic_print(CLP_LOG_LEVEL_ERROR, \
			"[%s][error] %s: "fmt, CLP_LOG_TAG, __func__, ##arg)
#define CLP_LOGW(fmt, arg...) \
	clp_dynamic_print(CLP_LOG_LEVEL_WARRING, \
			"[%s][warring] %s: "fmt, CLP_LOG_TAG, __func__, ##arg)
#define CLP_LOGI(fmt, arg...) \
	clp_dynamic_print(CLP_LOG_LEVEL_INFO, \
			"[%s][info] %s: "fmt, CLP_LOG_TAG, __func__, ##arg)
#define CLP_LOGD(fmt, arg...) \
	clp_dynamic_print(CLP_LOG_LEVEL_DEBUG, \
			"[%s][debug] %s: "fmt, CLP_LOG_TAG, __func__, ##arg)
...
static void plugin_clip_hook_t6(struct plugin_clip *p, u8 status)
{
	...
	if (status & (MXT_T6_STATUS_RESET|MXT_T6_STATUS_CAL)) {
		...
		CLP_LOGD("CLP hook T6 end\n");
	}

	CLP_LOGD("mxt clp flag=0x%lx %x\n", obs->flag, status);
}
...
/*
static int clip_pa_hook_t9_t100_points(struct plugin_clip *p,
		int id, int x, int y, u32 package, unsigned long flag)
{
...
static int clip_pa_hook_t9_t100_points_scraux(struct plugin_clip *p,
		struct scr_info *in, unsigned long flag)
{
	struct clp_config *cfg = p->cfg;
	struct clp_observer *obs = p->obs;
	struct pa_config *pa_cfg = &cfg->pa;
	...
	CLP_LOGD("scr num %d(%d) thld %d(%d) %d(%d) %d(%d) %d(%d) flag=0x%lx ret %d\n",
...
static int clip_cl_hook_t9_t100_points(struct plugin_clip *p,
...
static int clip_sup_hook_t9_t100_points(struct plugin_clip *p,
		int id, int x, int y, u8 area, unsigned long flag)
{
	struct clp_config *cfg = p->cfg;
	struct clp_observer *obs = p->obs;
...
static int clip_pos_hook_t9_t100_points(struct plugin_clip *p,
		int id, int x, int y, unsigned long flag)
{
	struct clp_config *cfg = p->cfg;
	struct clp_observer *obs = p->obs;
...
static int plugin_clip_hook_t9_t100(struct plugin_clip *p,
...
static void plugin_clip_reset_slots_hook(struct plugin_clip *p)
{
	...
	memset(&obs->touch_id_list,
			0,
			sizeof(*obs) - offsetof(struct clp_observer, touch_id_list));
			//clear data after touch_id_list
}
...
static ssize_t clip_pa_show(struct plugin_clip *p, char *buf, size_t count)
{
	...
	CLP_LOGI("pa status: Flag=0x%08lx\n", pa_obs->flag);
...
static ssize_t clip_cl_show(struct plugin_clip *p, char *buf, size_t count)
{
	...
	for (i = 0; i < MAX_TRACE_POINTS; i++) {
		CLP_LOGI("cl status[%d]: Flag=0x%08lx\n", i, cl_obs[i].flag);
	}
...
static ssize_t clip_sup_show(struct plugin_clip *p, char *buf, size_t count)
{
	...
	for (i = 0; i < MAX_TRACE_POINTS; i++) {
		CLP_LOGI("sup status[%d]: Flag=0x%08lx\n", i, sup_obs[i].flag);
	}
	...
	if (count > 0) {
		...
		for (i = 0; i < NUM_RESTICTS; i++) {
			offset += scnprintf(buf + offset,
					...
					"SUP range[%d]: %d %d\n",
					i,
					sup_cfg[i].range[RANGE_THLD],
					sup_cfg[i].range[RANGE_HYSIS]);
		}
		for (i = 0; i < NUM_RESTICTS; i++) {
			offset += scnprintf(buf + offset,
					count,
					"SUP range internal[%d]: %d %d\n",
					i,
					sup_cfg[i].range_internal[RANGE_THLD],
					sup_cfg[i].range_internal[RANGE_HYSIS]);
		}
...
static ssize_t clip_pos_show(struct plugin_clip *p, char *buf, size_t count)
{
	...
	CLP_LOGI("pos status: Flag=0x%08lx\n", pos_obs->flag);
	...
	if (count > 0) {
		for (i = 0; i < NUM_CLIP_POS_AREA; i++) {
			...
			offset += scnprintf(buf + offset,
					...
					i,
					pos_obs->state[i].sum,
					pos_obs->state[i].touch_id_list);
		}
	}
	return offset;
}

static void plugin_clip_pre_process_messages(struct plugin_clip *p,
		unsigned long pl_flag)
{
	...
	CLP_LOGD("mxt plugin_clip_pre_process_messages pl_flag=0x%lx flag=0x%lx\n",
			pl_flag, obs->flag);
}

static long plugin_clip_post_process_messages(struct plugin_clip *p,
		unsigned long pl_flag)
{
	struct clp_observer *obs = p->obs;
	...
	CLP_LOGD("mxt clp pl_flag=0x%lx flag=0x%lx\n", pl_flag, obs->flag);
...
ssize_t plugin_clip_show(struct plugin_clip *p, char *buf, size_t count)
{
	...
	CLP_LOGI("clip status: Flag=0x%08lx\n", obs->flag);

	CLP_LOGI("clip touch list %lx\n", obs->touch_id_list);

	for (i = 0; i < MAX_TRACE_POINTS; i++) {
		if (obs->first[i].x || obs->first[i].y)
			CLP_LOGI("first[%d]: %d,%d\n",
...
static int plugin_clp_debug_show(struct plugin_clip *p)
{
	...
	CLP_LOGI("PLUG_CLIP_VERSION: 0x%x\n", PLUG_CLIP_VERSION);
...
static int plugin_clp_debug_store(struct plugin_clip *p, const char *buf, size_t count)
{
	...
	if (sscanf(buf, "status: Flag=0x%lx\n",
		...
		if (count > 4 && count < sizeof(name)) {
			...
			CLP_LOGI("name %s, offset %d, ret %d\n", name, offset, ret);
			if (ret == 1) {
				if (strncmp(name, "pa", 2) == 0) {
					if (sscanf(buf + offset,
						...
						CLP_LOGE("Unknow clp sup command: %s\n",
								buf + offset);
					}
					...
					if (sscanf(buf + offset,
						...
						CLP_LOGE("Unknow clp cl command: %s\n", buf + offset);
					}
					...
					CLP_LOGE("Unknow clp command: %s\n", buf);
					return -EINVAL;
				}
			} else{
				CLP_LOGE("Unknow parameter, ret %d\n", ret);
			}
		}
	}
...
static int init_sup(struct plugin_clip *p)
{
	...
	for (i = 0; i < NUM_RESTICTS; i++) {
		...
		memcpy(sup_cfg[i].range,
				sup_range_list[i],
				sizeof(sup_cfg[i].range));

		memcpy(sup_cfg[i].range_internal,
				sup_range_internal_list[i],
				sizeof(sup_cfg[i].range_internal));
	}
...
static int init_clp_object(struct plugin_clip *p)
{
	...
	ret = init_pa(p);
	if (ret) {
		CLP_LOGE("Failed to init_pa\n");
		return -ENOMEM;
	}

	ret = init_cl(p);
	if (ret) {
		CLP_LOGE("Failed to init_cl\n");
		return -ENOMEM;
	}

	ret = init_sup(p);
	if (ret) {
		CLP_LOGE("Failed to init_sup\n");
		return -ENOMEM;
	}

	ret = init_pos(p);
	if (ret) {
		CLP_LOGE("Failed to init_pos\n");
		return -ENOMEM;
	}
...
{
	CLP_LOGI("plugin clip clp version 0x%x\n", PLUG_CLIP_VERSION);

	p->obs = kzalloc(sizeof(struct clp_observer), GFP_KERNEL);
	if (!p->obs) {
		CLP_LOGE("Failed to allocate memory for clp observer\n");
		return -ENOMEM;
	}

	p->cfg = kzalloc(sizeof(struct clp_config), GFP_KERNEL);
	if (!p->cfg) {
		CLP_LOGE("Failed to allocate memory for clp cfg\n");
		kfree(p->obs);
		p->obs = NULL;
		return -ENOMEM;
	}
