...
static struct stk3x1x_platform_data stk3x1x_pfdata_QCT={ 
  ...
  .ps_thd_l = 1500, 
  //.int_pin = 40,  
  /*modify int gpio setting tp mtp.dtsi */
  .transmittance = 1112, 
};
...
static int32_t stk3x1x_software_reset(struct stk3x1x_data *ps_data)
{
   ...
   w_reg =0x01;// 0x7F;
    ...
    if (w_reg != r)
    {
        printk(KERN_ERR "%s:software reset: read-back value is not the same w_reg = %d  r=%d \n", __func__,w_reg,r);
        return -1;
    }
...
static int32_t stk3x1x_init_all_setting(struct i2c_client *client, struct stk3x1x_platform_data *plat_data)
{
	...
	if(ret < 0)
		{
		printk("stk init ret = %d \n",ret);
		return ret;
		}
	
	//ensure sensing sensor & slave address not conflict 
...
//Colby add for Qualcomm device tree start
	...
	ps_data->pin_default =	pinctrl_lookup_state(ps_data->pinctrl, "default");
	if (IS_ERR_OR_NULL(ps_data->pin_default)) {
		printk("Failed to look up default state\n");
		return PTR_ERR(ps_data->pin_default);
	}

	err = pinctrl_select_state(ps_data->pinctrl, ps_data->pin_default);
	if (err) {
		printk("Can't select pinctrl default state\n");
		return err;
	}

	err = gpio_request(ps_data->int_pin, "stk-int");
	if (err < 0) {
		printk("gpio_request failed, err = %d", err);
		return err;
	}

	err = gpio_direction_input(ps_data->int_pin);
	if (err < 0) {
		printk("gpio_direction_input failed, err = %d", err);
		return err;
	}
#else
	err = gpio_request(ps_data->int_pin,"stk-int");
...
#endif /* #ifdef USE_LINUX_PIN_CONTROL */
#else
//Colby add for Qualcomm device tree end
	//err = gpio_request(ps_data->int_pin,"stk-int");
	//if(err < 0)
...
//-----------------------------power sets start---------------------------------------------
	...
	reg_lvs2 = regulator_get(&client->dev, "vdd");
	if (IS_ERR(reg_lvs2))
		printk("Regulator get failed vcc-ana rc=%ld\n", PTR_ERR(reg_lvs2));

	
	if (regulator_count_voltages(reg_lvs2) > 0) {
		error = regulator_set_voltage(reg_lvs2, 1800000, 1800000);
		if (error)
			printk("regulator set_vtg vdd failed rc=%d\n", error);
	}

	error = regulator_enable(reg_lvs2);
	if (error) {
...
//----------------------------------------------------------------------------------------
	...
	reg_l18 = regulator_get(&client->dev, "vio");
	if (IS_ERR(reg_l18))
		printk("Regular get failed vcc-lvs1, rc = %ld\n", PTR_ERR(reg_l18));


	if (regulator_count_voltages(reg_l18) > 0) {
		error = regulator_set_voltage(reg_l18, 2850000, 2850000);
		if (error)
			printk("regulator set_vtg vio failed rc=%d\n", error);
	}

	error = regulator_enable(reg_l18);
	if (error) {
		printk("enable vio failed, rc=%d\n", error);
		regulator_put(reg_l18);
	}
...
#endif
		 /* parse device tree contants */
		 plat_data->int_pin = of_get_named_gpio_flags(
			client->dev.of_node,
			"stk,irq-gpio",
			0,
			NULL);
...
#endif
	printk(KERN_INFO "%s: STK probe successfully", __func__);
	return 0;
